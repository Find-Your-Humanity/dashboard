  ...
  if (error) {
    return (
      <Box sx={{ p: 3 }}>
        ...
        <Button variant="contained" onClick={fetchBillingData}>
          다시 시도
        </Button>
      </Box>
    );
          ...
          <Grid item xs={12} md={4} key={plan.id}>
            <Card 
              sx={{ 
                ...
                border: currentPlanData?.plan.id === plan.id ? 2 : 1,
                borderColor: currentPlanData?.plan.id === plan.id ? 'primary.main' : 'divider'
              }}
            >
              <CardContent sx={{ flexGrow: 1, display: 'flex', flexDirection: 'column' }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                  ...
                  {plan.is_popular && (
                    <Chip label="인기" color="primary" size="small" />
                  )}
                </Box>
